...
namespace ZHSM
{
    public static class SoundExtension
    {
        ...
        public static int? PlayMusic(this SoundComponent soundComponent, int musicId, object userData = null)
        {
            ...
            {
                var tbMusic = tables.TbMusicCfg.Get(musicId);
                if (tbMusic == null)
                {
        ...
        public static int? PlaySound(this SoundComponent soundComponent, int soundId, Vector3 worldPosition, object userData = null)
        {
            if (GameEntry.LubanTable.TryGetTables(out cfg.Tables tables))
            {
                var tbSound = tables.TbSoundCfg.Get(soundId);
                ...
                return soundComponent.PlaySound(AssetUtility.GetSoundAsset(tbSound.AssetName), "Sound", Constant.AssetPriority.SoundAsset, playSoundParams, worldPosition, userData);
            }

            return null;
        }
        
        public static int? PlaySound(this SoundComponent soundComponent, int[] soundIds, Vector3 worldPosition, object userData = null)
        {
            if (soundIds == null || soundIds.Length <= 0) return null;
            
            if (GameEntry.LubanTable.TryGetTables(out cfg.Tables tables))
            {
                int soundId = GetRandomSoundId(soundComponent, soundIds);
                var tbSound = tables.TbSoundCfg.Get(soundId);
        ...
        public static int GetRandomSoundId(this SoundComponent soundComponent, int[] soundIds)
        {
            if (soundIds == null || soundIds.Length <= 0) return 0;
            
            return soundIds[UnityEngine.Random.Range(0, soundIds.Length)];
        }
        ...
        public static int? PlayUISound(this SoundComponent soundComponent, int uiSoundId, object userData = null)
        {
            if (GameEntry.LubanTable.TryGetTables(out cfg.Tables tables))
            {
                var uiSound = tables.TbUISoundCfg.Get(uiSoundId);

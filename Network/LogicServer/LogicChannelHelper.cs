...
{
    public class LogicChannelHelper : INetworkChannelHelper
    {
        ...
        /// 获取消息包头长度。
        /// </summary>
        public int PacketHeaderLength => 8;
        ...
        {
            CSHeartbeat heartbeat = ReferencePool.Acquire<CSHeartbeat>();
            heartbeat.FrameCount = Time.frameCount;
            
            // Log.Info($"[Network] 发送心跳 {heartbeat.FrameCount}");
            m_NetworkChannel.Send(heartbeat);
            return true;
        }
        ...
        public bool Serialize<T>(T packet, Stream destination) where T : Packet
        {
            ...
            m_CachedStream.Write(m_CsHeaderBuffer, 0, PacketHeaderLength);
        ...
        public Packet DeserializePacket(IPacketHeader packetHeader, Stream source, out object customErrorData)
        {
            ...
            if (scPacketHeader.IsValid)
            {
                ...
                if (packetType != null)
                {
                    packet = (Packet)RuntimeTypeModel.Default.Deserialize(source, ReferencePool.Acquire(packetType),
                        packetType, packetHeader.PacketLength);
                }
                else
                {

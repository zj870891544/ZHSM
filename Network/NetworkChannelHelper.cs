...
namespace ZHSM
{
    public class NetworkChannelHelper : INetworkChannelHelper
    {
        ...
        private byte[] m_CsHeaderBuffer = null;
        private byte[] m_ScHeaderBuffer = null;
        private INetworkChannel m_NetworkChannel = null;

        /// <summary>
        /// 获取消息包头长度。
        /// </summary>
        public int PacketHeaderLength => 16;
        ...
        public void Initialize(INetworkChannel networkChannel)
        {
            ...
            m_CsHeaderBuffer = new byte[PacketHeaderLength];
            m_ScHeaderBuffer = new byte[PacketHeaderLength];
        ...
        {
            BrigeHeartbeatRequest heartbeatRequest = ReferencePool.Acquire<BrigeHeartbeatRequest>();
            heartbeatRequest.FrameCount = 50;
            
            m_NetworkChannel.Send(heartbeatRequest);
            return true;
        }
        ...
        public bool Serialize<T>(T packet, Stream destination) where T : Packet
        {
            ...
            //写入数据头
            int packetLength = (int)m_CachedStream.Length - PacketHeaderLength;
            BitConverter.GetBytes(packetImpl.Id).CopyTo(m_CsHeaderBuffer, 0);//协议ID
            BitConverter.GetBytes(packetLength).CopyTo(m_CsHeaderBuffer, 4); //数据包长度
            BitConverter.GetBytes(1).CopyTo(m_CsHeaderBuffer, 8); //协议版本号
            BitConverter.GetBytes(0).CopyTo(m_CsHeaderBuffer, 12); //附加信息
            m_CachedStream.Position = 0L;
            m_CachedStream.Write(m_CsHeaderBuffer, 0, m_CsHeaderBuffer.Length);
            
            Log.Info($"发送数据包 {packet.Id}  length:{packetLength}");
        ...
        {
            SCPacketHeader scPacketHeader = ReferencePool.Acquire<SCPacketHeader>();
            
            source.Read(m_ScHeaderBuffer, 0, 4);
            scPacketHeader.Id = BitConverter.ToInt32(m_ScHeaderBuffer, 0);
            
            source.Read(m_ScHeaderBuffer, 4, 4);
            scPacketHeader.PacketLength = BitConverter.ToInt32(m_ScHeaderBuffer, 4);
            
            source.Read(m_ScHeaderBuffer, 8, 4);
            scPacketHeader.Version = BitConverter.ToInt32(m_ScHeaderBuffer, 8);
            
            source.Read(m_ScHeaderBuffer, 12, 4);
            scPacketHeader.ExCode = BitConverter.ToInt32(m_ScHeaderBuffer, 12);
            
            Log.Info($"收到消息 id:{scPacketHeader.Id} length:{scPacketHeader.PacketLength} version:{scPacketHeader.Version}");
        ...
        {
            Log.Info("反序列化消息 >>> DeserializePacket");
